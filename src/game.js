...
export class Game {
  ...
  showMenu(type) {
    ...
    if (type === 'settings') content = `<div class="menu-eyebrow">SETTINGS</div><h2 class="menu-title">画面与声音</h2>
      ...
      <label class="setting-row">画面品质<select aria-label="画面品质" id="quality-setting"><option value="high" ${this.quality !== 'low' ? 'selected' : ''}>精致 · CG 像素</option><option value="low" ${this.quality === 'low' ? 'selected' : ''}>流畅 · 清晰像素</option></select></label>
      <p class="settings-note">重击时配乐会轻轻退后，让圣锤的声音更清楚。<br>音量与配乐选择会自动保存。<br>精致画质保留更细的像素、柔和阴影和金属受光；手机默认使用清晰的轻量画质，保留人物与街道细节。</p>${btn('back','返 回',true)}`;
    if (type === 'dead') {
